...
const SHARD = preload("res://destroyable_objects/shard.tscn")

@export_range(0, 200) var nbr_of_shards: int = 20 #sets the number of break points
@export var threshhold: float = 10.0 #prevents slim triangles being created at the sprite edges
@export var min_impulse: float = 50.0 #impuls of the shards upon breaking
...
func _ready() -> void:
	delete_timer.timeout.connect(on_delete_timer_timeout)
	...
	if get_parent() is Sprite2D:
		...
		for i in nbr_of_shards:
			...
			elif p.y > _rect.position.y - threshhold:
				p.y = _rect.end.y
			points.append(p)
